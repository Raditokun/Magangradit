...
func (r *kendaraanRepository) GetAll() ([]model.Kendaraan, error) {
	query := `
	SELECT id, id_jenis_kendaraan, nopol, no_bpkb, no_mesin, no_rangka, nama_kendaraan, warna, kapasitas, deskripsi, status, created_at, created_by, updated_at, updated_by
	FROM kendaraan
	...
	for rows.Next() {
		...
		if err := rows.Scan(&k.ID, &k.IdJenisKendaraan, &k.Nopol, &k.NoBpkb, &k.NoMesin, &k.NoRangka, &k.NamaKendaraan, &k.Warna, &k.Kapasitas, &k.Deskripsi, &k.Status, &k.CreatedAt, &k.CreatedBy, &k.UpdatedAt, &k.UpdatedBy); err != nil {
			return nil, err
		}
...
func (r *kendaraanRepository) Update(id string, req model.UpdateKendaraanRequest) (int64, error) {
	...
	result, err := database.DB.Exec(query, req.IdJenisKendaraan, req.Nopol, req.NoBpkb, req.NoMesin, req.NamaKendaraan, req.Warna, req.Kapasitas, req.Deskripsi, req.Status, time.Now(), req.UpdatedBy, id)
	if err != nil {
		return 0, err
	}
